...
$sql = "SELECT * FROM `session_tbls2018` INNER JOIN `role_tbls` ON  `session_tbls2018`.`sessionNo` = `role_tbls`.`sessionNo` AND `session_tbls2018`.`class` = `role_tbls`.`class` AND `session_tbls2018`.`year` = `role_tbls`.`year` WHERE `role_tbls`.`dr_tbl_id` = :dr_tbl_id AND `role_tbls`.`year` = '" . $this_year . "' ORDER BY `beginDate`;";

$stmt = $pdo->prepare( $sql );
$stmt->bindValue( ":dr_tbl_id", $_POST[ 'dr_tbl_id' ], PDO::PARAM_INT );
$flag = $stmt->execute();
...
foreach ( $rows as $row ) {
	...
	$search_results .= $i;
	$search_results .= '</td>';
	if ( $row[ 'day' ] == '2018-12-15' ) {
		$search_results .= '<td class="small_left_align red">';
	} else {
		$search_results .= '<td class="small_left_align">';
	}
	if ( $row[ 'class' ] == 'zagaku' ) {
		$search_results .= '12/15 ZAGAKU (インタベ座学)';
	} else if ( $row[ 'class' ] == 'com' ) {
		$search_results .= '12/16 Comedical (コメディカル・セッション)';
	} else if ( ( $row[ 'day' ] == '2018-12-15' ) && ( $row[ 'class' ] == 'tri' ) ) {
		$search_results .= '12/15 TRI Session';
	} else {
		$search_results .= '12/16 TRI Session';
	}
	$search_results .= '</td>';
...
<body>
	<div class="container">
		<div class="row col-md-12 text-center">
			...
			<div class="col-sm-12">
				<p><a class="btn btn-danger" id="return" href="../index2018.php">Return</a></p>
			</div>
		</div>
