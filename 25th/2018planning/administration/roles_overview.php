<?php
	...
	$stmt = $pdo->prepare("SELECT * FROM `session_tbls2018`WHERE `class` = 'com' ORDER BY `sessionNo` ASC;");
	$flag = $stmt->execute();
	...
	$stmt = $pdo->prepare("SELECT * FROM `session_tbls2018` WHERE `sessionNo` > '0' AND `sessionNo`< '20' AND `class` = 'com' ORDER BY `sessionNo` ASC;");
	$stmt->execute();
	$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
