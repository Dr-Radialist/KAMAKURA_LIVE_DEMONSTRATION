<?php
	...
	$stmt = $pdo->prepare("SELECT * FROM `session_tbls2019` WHERE `sessionNo` = :sessionNo AND `class` = :class AND `year` = :year;");
  ...
  <div class="row">
    <div class="row">
      <div class="col-lg-12">
        ...
        <h4>座長:
          <?=  _Q(makeTriList($pdo, $row['sessionNo'], 1, $class, $this_year)) ?>
        </h4>
        <?php 
			if ($row['sessionTitle'] != '') {
		 ...
		 ?>
        <h4>モデレーター:
          <?= _Q(makeTriList($pdo, $row['sessionNo'], 2, $class, $this_year)) ?>
        </h4>
        <?php
			if ($row['lectureTitle'] != '') {
		...
		?>
        <h4>演者:
          <?= _Q(makeTriList($pdo, $row['sessionNo'], 3, $class, $this_year)) ?>
        </h4>
        <h5>カテ室画像診断者:
          <?= _Q(makeTriList($pdo, $row['sessionNo'], 4, $class, $this_year)) ?>
        </h5>
        <?php
			if ($row['venue'] != '') {
