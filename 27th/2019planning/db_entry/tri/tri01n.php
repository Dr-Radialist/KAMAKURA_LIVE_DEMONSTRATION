...
$stmt = $pdo->prepare( "SELECT * FROM `session_tbls2019` WHERE `sessionNo` = :sessionNo  AND `class` = :class AND `year` = '" . $this_year . "';" );
...
foreach ( $interpreters as $interpreter ) {
	...
	if ( !isset( $interpreter[ 'kanji_sirname' ] ) || $interpreter[ 'kanji_sirname' ] == "" ) {
		...
		$interpreter_list .= "(" . $interpreter[ 'hp_name_english' ] . "), ";
	} else {
...
$stmt = $pdo->prepare( $sql );
$stmt->bindValue( ":sessionNo", $_POST[ 'sessionNo' ], PDO::PARAM_INT );
$stmt->bindValue( ":role_kind", '6', pdo::PARAM_INT );
$stmt->bindValue( ":class", 'tri', PDO::PARAM_STR );
$stmt->execute();
$operators = $stmt->fetchAll( PDO::FETCH_ASSOC );
$operator_list = '';
foreach ( $operators as $operator ) {
	$operator_list .= '<form class="form-inline" method="post" action="delete_role.php">';
	$operator_list .= '<button type="submit" id="delete_role" /><i class="glyphicon glyphicon-remove-sign"></i></button>';
	$operator_list .= '<input type="hidden" name="id" value="' . $operator[ 'id' ] . '" />';
	if ( !isset( $interpreter[ 'kanji_sirname' ] ) || $interpreter[ 'kanji_sirname' ] == "" ) {
		$operator_list .= $operator[ 'english_sirname' ] . " " . $operator[ 'english_firstname' ];
		$operator_list .= "(" . $operator[ 'hp_name_english' ] . "), ";
	} else {
		$operator_list .= $operator[ 'kanji_sirname' ] . " " . $operator[ 'kanji_firstname' ];
		$operator_list .= "(" . $operator[ 'hp_name_japanese' ] . "), ";
	}
	$operator_list .= '</form>';
}
$operator_list = rtrim( $operator_list, ', ' );
?>
...
<body>
	<div class="container">
		<div class=" col-lg-10">
			<h1>TRIセッション入力・修正フォーム
			<button class="btn btn-sm btn-info" onClick="location.href='../../../../planner_only/index-mod.php';">緊急脱出</button>
		</h1>
			<div class="row form-inline">
				<form name="come" method="post" action="tri02n.php">
					<div class="col-lg-5">
						<input type="submit" class="btn btn-danger btn-lg" value="入力/修正/終了"/><br><br>
					</div>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="begin" placeholder="開始時刻を入力して下さい" value="<?php if (($row['begin']) != '')  echo _Q(mb_substr($row['begin'], 0, 5)); ?>"/>
					</div>
					<div class="form-group">
						<label for="sessionTime">セッション開催時間(分)<span class="req">必須</span></label>
						<input type="text" class="form-control" name="duration" placeholder="セッション持続時間(分)を入力して下さい" value="<?php if (($row['duration']) != '')  echo _Q($row['duration']); ?>" required/>
					</div>
					<div class="form-group">
						<label for="sessionTitle">セッションタイトル<span class="req">必須</span></label>
						<input type="text" class="form-control" name="sessionTitle" placeholder="sessionタイトルを入力して下さい(必須入力)" value="<?php if (($row['sessionTitle']) != '')  echo _Q($row['sessionTitle']); ?>" required/>
					</div>

					<div class="form-group">
						<label for="lectureTitle">講演演題名</label>
						<input type="text" class="form-control" name="lectureTitle" placeholder="講演演題名を入力して下さい" value="<?php if (($row['lectureTitle']) != '')  echo _Q($row['lectureTitle']); ?>"/>
					</div>
					<div class="form-group">
						<label for="venue">会場</label>
						<input type="text" class="form-control" name="venue" placeholder="会場名を入力して下さい" value="<?php if (($row['venue']) != '')  echo _Q($row['venue']); ?>"/>
					</div>
					<div class="form-group">
						<label for="description">セッション内容</label>
						<textarea class="form-control" name="description" rows="6" cols="60" placeholder="セッションの内容について記載してください"><?= _Q($row['description']); ?>
</textarea>
					</div>
					<div class="form-group">
						<label for="sessionContent">共催企業</label>
						<input type="text" class="form-control" name="cosponsor" placeholder="共催企業があれば、入力して下さい" value="<?php if (($row['cosponsor']) != '')  echo _Q($row['cosponsor']); ?>"/>
					</div>
					<br><br>
			...
			<div class="row form-group form-inline">
				<div class="col-sm-6">
					<div class="text-left">
						<div style="display:inline-flex;">
							<form method="post" action="tri_chair_mod_add01new.php" class="form-inline text-left" style="margin-right: 0.3em;">
								<button type="submit" class="btn btn-sm btn-warning" name="session_tbl_id" value="<?= $row['id'] ?>">座長追加</button>
								<input type="hidden" name="role_kind" value="1"/>
								<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
							</form>
							<form method="post" action="tri_chair_role-add01new00.php" class="form-inline text-left">
								<button type="submit" class="btn btn-sm btn-default" name="session_tbl_id" value="<?= $row['id'] ?>">座長追加 (Role数)</button>
								<input type="hidden" name="role_kind" value="1"/>
								<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
							</form>
						</div>
					...
					<div style="display:inline-flex;">
						<form method="post" action="tri_chair_mod_add01new.php" class="form-inline text-left" style="margin-right: 0.3em;">
							<button type="submit" class="btn btn-sm btn-danger" name="session_tbl_id" value="<?= $row['id'] ?>">講師追加</button>
							<input type="hidden" name="role_kind" value="3"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
						<form method="post" action="tri_chair_role-add01new00.php" class="form-inline text-left">
							<button type="submit" class="btn btn-sm btn-default" name="session_tbl_id" value="<?= $row['id'] ?>">講師追加 (Role数)</button>
							<input type="hidden" name="role_kind" value="3"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
					</div>
					<div class="text-left">Lecturer:
						<?= $lecturer_list ?>
					</div><br><br>
					<div style="display:inline-flex;">
						<form method="post" action="tri_chair_mod_add01new.php" class="form-inline text-left" style="margin-right: 0.3em;">
							<button type="submit" class="btn btn-sm btn-info" name="session_tbl_id" value="<?= $row['id'] ?>">Operator追加</button>
							<input type="hidden" name="role_kind" value="6"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
						<form method="post" action="tri_chair_role-add01new00.php" class="form-inline text-left">
							<button type="submit" class="btn btn-sm btn-default" name="session_tbl_id" value="<?= $row['id'] ?>">Operator追加 (Role数)</button>
							<input type="hidden" name="role_kind" value="6"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
					</div>
					<div class="text-left">Operator:
						<?= $operator_list ?>
					</div><br><br>
				...
				<div class="col-sm-6">
					<div style="display:inline-flex;">
						<form method="post" action="tri_chair_mod_add01new.php" class="form-inline text-left" style="margin-right: 0.3em;">
							<button type="submit" class="btn btn-sm btn-success" name="session_tbl_id" value="<?= $row['id'] ?>">Discussant追加</button>
							<input type="hidden" name="role_kind" value="2"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
						<form method="post" action="tri_chair_role-add01new00.php" class="form-inline text-left">
							<button type="submit" class="btn btn-sm btn-default" name="session_tbl_id" value="<?= $row['id'] ?>">Discussant追加 (Role数)</button>
							<input type="hidden" name="role_kind" value="2"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
					</div>
					<div class="text-left">Discussant:
						<?= $moderator_list ?>
					</div><br><br>

					<div style="display:inline-flex;">
						<form method="post" action="tri_chair_mod_add01new.php" class="form-inline text-left" style="margin-right: 0.3em;">
							<button type="submit" class="btn btn-sm btn-primary" name="session_tbl_id" value="<?= $row['id'] ?>">カテ室画像診断追加</button>
							<input type="hidden" name="role_kind" value="4"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
						<form method="post" action="tri_chair_role-add01new00.php" class="form-inline text-left">
							<button type="submit" class="btn btn-sm btn-default" name="session_tbl_id" value="<?= $row['id'] ?>">カテ室画像診断追加 (Role数)</button>
							<input type="hidden" name="role_kind" value="4"/>
							<input type="hidden" name="sessionNo" value="<?= _Q($row['sessionNo']); ?>"/>
						</form>
					</div>
					<div class="text-left">カテ室画像診断:
						<?= $interpreter_list ?>
					</div><br><br>
		...
		<hr>
		<div class="col-lg-10">
			<button class="btn btn-lg btn-info" onClick="location.href='../../../../planner_only/index-mod.php';">緊急脱出</button>
			<br><br>
		</div>
		<footer>
