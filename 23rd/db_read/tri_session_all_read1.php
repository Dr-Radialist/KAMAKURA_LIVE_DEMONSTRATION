<?php
	...
	/*
	$stmt = $pdo->prepare("SELECT * FROM `tri_session_tbls` WHERE '1' = '1';");
	$flag = $stmt->execute();
	...
	usort($rows, function ($a, $b) {						// 多次元配列をsessionNoでソートする
    		return $a['sessionNo'] - $b['sessionNo'];		// http://qiita.com/shy_azusa/items/54dadc55e3e71cde1445
	});
	*/
	$stmt = $pdo->prepare("SELECT * FROM `tri_session_tbls`WHERE '1' = '1' ORDER BY `sessionNo` ASC;");
	$flag = $stmt->execute();
	$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
	if (!$flag) {
    		$infor = $stmt->errorInfo();
	...
	$stmt = $pdo->prepare("SELECT MAX(`changed`) FROM `tri_session_tbls`;");
	$stmt->execute();
    ...
    <ul class="nav nav-pills">
      <li class="dropdown"> <a href="#" class="dropdown-toggle white" data-toggle="dropdown">LIVE2016<b class="caret"></b></a>
        <ul class="dropdown-menu">
          ...
          <li><a href="../22nd/program_detail/faculty.html">Faculty</a></li>
          <li><a href="#" id="">???</a></li>
        </ul>
      </li>
      ...
      <div>
        <div  class="session_title">
          <form method="post" action="tri_sessions.php">
            ...
            <div>
              <div class="fleft"><span class="smf"> [
                <?= _Q($rows[1]['sessionNo']) ; ?>
                ] </span>
