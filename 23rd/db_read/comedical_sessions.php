    ...
    <div class="row">
      ...
      <form name="tri" method="post" action="comedical04n.php">
        <div class="col-lg-12">
          ...
          <h4>座長:
            <?=  _Q(makeComeList($pdo, $row['sessionNo'], 1)) ?>
          </h4>
          <?php 
			if ($row['sessionTitle'] != '') {
		 ...
		 ?>
        <h4>モデレーター: 
        		<?= _Q(makeComeList($pdo, $row['sessionNo'], 2)) ?>
         </h4>
          <?php
			if ($row['lectureTitle'] != '') {
		...
		?>
        <h4>演者:
        		<?= _Q(makeComeList($pdo, $row['sessionNo'], 3)) ?>
        </h4>
        <h5>カテ室画像診断者:
        		<?= _Q(makeComeList($pdo, $row['sessionNo'], 4)) ?>
         </h5>
       ...
       <!-- <p><a class="btn btn-primary btn-lg" href="come_session_all_read.php">戻る　&raquo;</a></p> -->
