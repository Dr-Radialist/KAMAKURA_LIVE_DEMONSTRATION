<?php
	...
	$stmto = $pdo->prepare("SELECT * FROM `session_tbls` WHERE `id` = :id AND `class` =:class;");
	$stmto->bindValue(":id", $_POST['id'], PDO::PARAM_INT);
	$stmto->bindValue(":class", 'evt', PDO::PARAM_STR);
	$flago = $stmto->execute();
	if (!$flago) {
	...
	if ($change_flag) {
		...
		"`description` = :description, `cosponsor` = :cosponsor, `class` = :class, `changed` = :changed WHERE `id` = :id;");
	...
	header('Location:../../index2016evt_mod-n.php');
?>
