...
非公開用のものは 2016planning/に同名ファイルで存在する
*/
	 ...
	 if (!isset($_GET['member_kind'])) {
		$stmt = $pdo->prepare("SELECT * FROM `doctor_tbls` WHERE '1' = '1' ORDER BY `member_kind` DESC;");
		 ...
		 if (!is_numeric($_GET['member_kind'])) {
			...
			$stmt = $pdo->prepare("SELECT * FROM `doctor_tbls` WHERE`member_kind` = :member_kind ORDER BY `member_kind` DESC;");
	...
	$title = "KAMAKURA LIVE 2016 Faculty List";
	if ($_SESSION['member_kind'] == 1) $title = "NPO社員 2016";
	if ($_SESSION['member_kind'] == 2) $title = "NPO年次社員2016";
	if ($_SESSION['member_kind'] == 3) $title = "Foreign Faculty 2016";
	if ($_SESSION['member_kind'] == 4) $title = "国内招聘Faculty 2016";
	if ($_SESSION['member_kind'] == 5) $title = "親善参加";
	if ($_SESSION['member_kind'] == 6) $title = "Sd Faculty 2016";
...
<script src="2016planning/dr_registration.js"></script>
</body>
</html>
