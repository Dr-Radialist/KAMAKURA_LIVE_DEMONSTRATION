<?php
	...
	foreach ($rows as $row) {
		...
		$result .= "<td class='small_left_align'>";
		if (_Q(makeComeList2($pdo, $i, 2)) != "") {
			$result .= _Q(makeComeList2($pdo, $i, 2));
			if (_Q(makeComeList2($pdo, $i, 3)) != "") {
				$result .= "<br>"._Q(makeComeList2($pdo, $i, 3));
			}
		} else {
			if (_Q(makeComeList2($pdo, $i, 3)) != "") $result .= _Q(makeComeList2($pdo, $i, 3));
		}
		$result .= "</td></td>";
...
<div class="container">
  ...
  <table class="table table-bordered small">
    <thead>
    <caption><h3 class="text-center text-danger">Comedical Session Overview 12/18 (SUN) <span style="color:red; font-weight:bold; font-size:small;">&nbsp;&nbsp;最終更新日時:
<?= $latest ?>
</span></h3></caption>
      <tr>
        <th class="small col-sm-1">時刻</th>
        <th class="small col-sm-2">Session</th>
        <th class="small col-sm-4">座長</th>
