...
// https://qiita.com/webpicker/items/a06955dae6f1014db412
//
$fp = fopen( 'tmp/' . $now . 'come_list.csv', 'w' );
$header = "Content-Type: text/plain";
// http://yarunaramaeni.com/blogs/4/
/*
        $fp = fopen($filename, 'w');
...
fputcsv( $fp, $title_row );
for ( $sessionNo = 1; $sessionNo < 12; $sessionNo++ ) {
  if ( $sessionNo != 9 ) {
    $data_array = [
      "講演番号; " . $sessionNo,
...
//文書の取得と文書のエンコード
$doc1 = file_get_contents( 'tmp/' . $now . 'come_list.csv' );
$doc_encode64_001 = chunk_split( base64_encode( $doc1 ) );
$doc2 = file_get_contents( 'tmp/' . $now . 'come_list.csv' );
