<?php
	...
	if (isset($_POST['chair_non_determine'])&&($_POST['chair_non_determine'] == true)) {
		$_POST['chair'] = "未定";
		$_SESSION['chair'] = "未定";
	} else {
		if (!isset($_POST['chair'])||($_POST['chair'] == '')) {
			...
			if (strlen($_POST['chair']) > 199) $error .= '座長が長すぎます!<br>';
		}
	}
	...
	if (!isset($_POST['moderator'])) {
		...
		$_SESSION['moderator'] = $_POST['moderator'];
		if (strlen($_POST['moderator']) > 499) $error .= 'Moderatorが長すぎます!<br>';
	}
	...
	if (!isset($_POST['speaker'])) {
		...
		if (strlen($_POST['speaker']) > 199) $error .= '演者名が長すぎます!<br>';
	}
	
	if (!isset($_POST['interpreter'])) {
		$_SESSION['interpreter'] = '';
	} else {
		$_SESSION['interpreter'] = $_POST['interpreter'];
		if (strlen($_POST['interpreter']) > 99) $error .= 'カテ室画像診断者の名前が長すぎます!<br>';
	}
	...
	if (!isset($_POST['cosponsor'])) {
		...
		if (strlen($_POST['cosponsor']) > 99) $error .= '共催企業が長すぎます!<br>';
	}
