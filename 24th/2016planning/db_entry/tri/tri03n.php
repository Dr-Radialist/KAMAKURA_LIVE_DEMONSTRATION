...
<div class="container">
  ...
  <div class="container">
    ...
    <div class="row">
      <form name="tri" method="post" action="tri04n.php">
         <div class="col-lg-12">
          <h3>セッション番号:
          ...
          <h4>座長:
            <?=  _Q(makeTriList($pdo, $_SESSION['sessionNo'], 1)) ?>
          </h4>
          <?php 
		 ...
		 ?>
        <h4>モデレーター: 
        		<?= _Q(makeTriList($pdo, $_SESSION['sessionNo'], 2)) ?>
         </h4>
          <?php
		...
		?>
        <h4>演者:
        		<?= _Q(makeTriList($pdo, $_SESSION['sessionNo'], 3)) ?>
        </h4>
        <h5>カテ室画像診断者:
        		<?= _Q(makeTriList($pdo, $_SESSION['sessionNo'], 4)) ?>
         </h5>
        ...
        <input type="hidden" name="chair" value="<?= _Q($_SESSION['chair']); ?>"  />
        <input type="hidden" name="moderator" value="<?= _Q($_SESSION['moderator']); ?>" />
        <input type="hidden" name="lectureTitle" value="<?= _Q($_SESSION['lectureTitle']); ?>" />
        <input type="hidden" name="speaker" value="<?= _Q($_SESSION['speaker']); ?>" />
        <input type="hidden" name="interpreter" value="<?= _Q($_SESSION['interpreter']); ?>" />
      ...
      <form method="post" action="tri01n.php">
